 ...
 * @category    modules
 *
 * @copyright 2017-2021 CCBill <http://www.ccbill.com/>
 * @author CCBill <support@ccbill.com>
 *
 * @version $Revision: 2 $ $Date: 2018-09-12 15:07:07 -0700 (Ср, 01 апр 2018) $ $Author: ccbill $
 */
class CcbillModel extends PaymentDriverModel
{
  //$install_lang["system_field_client_accnum"] = "Client Account No";
    public $payment_data = array(
        'gid'           => 'ccbill',
        'name'          => 'CCBill',
        'settings_data' => 'a:6:{s:13:"ccbill_client_accnum";s:6:"XXXXXX";s:13:"ccbill_client_subacc";s:4:"XXXX";s:4:"ccbill_salt";s:0:"";s:9:"ccbill_form_name";s:0:"";s:13:"ccbill_is_flexform";s:0:"";s:13:"ccbill_currency_code";s:3:"840"}',
        'logo'          => 'logo_ccbill.png',
    );
    public $settings = array(
        "ccbill_client_accnum" => array("type" => "text", "content" => "string", "size" => "middle"),
        "ccbill_client_subacc" => array("type" => "text", "content" => "string", "size" => "middle"),
        "ccbill_salt"          => array("type" => "text", "content" => "string", "size" => "middle"),
        "ccbill_form_name"     => array("type" => "text", "content" => "string", "size" => "middle"),
        "ccbill_is_flexform"   => array("type" => "text", "content" => "string", "size" => "middle"),
        "ccbill_currency_code" => array("type" => "text", "content" => "string", "size" => "middle"),
    );
    ...
    public function funcRequest($payment_data, $system_settings)
    {
        ...
        if ( !($payment_data["amount"] > 0) ) {
            ...
            $return["errors"][0] = 'Invalid Amount';
    ...
    public function funcResponce($payment_data, $system_settings)
    {
      ...
      if(isset($payment_data['initialPrice']))     $orderAmount       = $payment_data['initialPrice'];
      if(isset($payment_data['formPrice']))        $orderAmount       = $payment_data['formPrice'];
    ...
    private function verifyData($payment_data, $system_settings)
    {
      ...
      */
        ...
        if($myAction == "Approval_Post" || $myAction == "Denial_Post"){
          ...
          if(isset($payment_data['subscription_id'])) $subscriptionId = $payment_data['subscription_id'];
          if(isset($payment_data['subscriptionId']))  $subscriptionId = $payment_data['subscriptionId'];

          if($myAction == 'Approval_Post' && isset($payment_data['email']) && $subscriptionId != 0 ){

              $mySuccess = 1;
              $result = 'VERIFIED';

              if(isset($payment_data['subscription_id']))  $subscriptionId = $payment_data['subscription_id'];
              if(isset($payment_data['customer_fname']))   $myFirstName    = $payment_data['customer_fname'];
              if(isset($payment_data['customer_lname']))   $myLastName     = $payment_data['customer_lname'];
              if(isset($payment_data['email']))            $myEmail        = $payment_data['email'];
              if(isset($payment_data['accountingAmount'])) $myAmount       = $payment_data['accountingAmount'];
              if(isset($payment_data['initialPrice']))     $myAmount       = $payment_data['initialPrice'];
              if(isset($payment_data['formPrice']))        $myAmount       = $payment_data['formPrice'];
              //if(isset($payment_data['currencyCode']))     $myCurrencyCode = $payment_data['currencyCode'];
              if(isset($payment_data['zc_orderid']))       $myOrderId      = $payment_data['zc_orderid'];
              if(isset($payment_data['cardType']))         $cardType       = $payment_data['cardType'];
              if(isset($payment_data['responseDigest']))   $responseDigest = $payment_data['responseDigest'];
